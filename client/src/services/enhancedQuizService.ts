...
class EnhancedQuizService {
  ...
  async generateEnhancedQuiz(topic: string): Promise<EnhancedQuizResponse> {
    try {
      ...
      // For now, use the regular endpoint but with enhanced prompt
      // TODO: Update backend to support wikipediaContext parameter
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail?.error || 'Failed to generate enhanced quiz');
      }
   ...
   * Create enhanced prompt with Wikipedia data
   */
  private createEnhancedPrompt(topic: string, context: WikipediaContext): string {
    ...
    if (context.keyFacts.length > 0) {
      prompt += `\n\nKey Facts from Wikipedia:\n${context.keyFacts.slice(0, 10).map(fact => `- ${fact}`).join('\n')}`;
    }
  ...
  private extractKeyFacts(text: string): string[] {
    ...
    const keyIndicators = [
      'is a', 'are a', 'is the', 'are the', 'was', 'were', 'has', 'have', 'can', 'cannot',
      'includes', 'consists of', 'refers to', 'means', 'defined as', 'known as'
    ];
  ...
  private createSummary(articles: WikipediaArticle[]): string {
    ...
    if (articles.length > 1) {
      const additionalFacts = articles.slice(1)
   ...
   */
  private enhanceQuizWithSources(quizData: any, context: WikipediaContext): EnhancedQuizResponse {
    const enhancedQuiz = {
      ...quizData,
   ...
   */
  private async generateRegularQuiz(topic: string): Promise<EnhancedQuizResponse> {
    const response = await fetch(`${this.API_BASE_URL}/quiz/generate`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
